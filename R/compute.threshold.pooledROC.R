...
function(object, criterion = c("FPF", "TPF", "YI"), FPF, TPF, ci.level = 0.95, parallel = c("no", "multicore", "snow"), ncpus = 1, cl = NULL) {
	...
	if(criterion == "YI") {
		...
		res <- eval(parse(text = method))(object = object, TPF = TPF, ci.level = ci.level, parallel = parallel, ncpus = ncpus, cl = cl)
	}
	res$call <- match.call()
	res
}
