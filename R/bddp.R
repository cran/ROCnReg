...
function(y, X, prior, mcmc, standardise = TRUE) {
    ...
    for(i in 2:nsim) {
        ...
        for(l in 1:L) {
            X_l  <- matrix(X[z[i,] == l, ], ncol = k, nrow = ns[l])
            V <- solve(Sigmainv[i-1,,] + tau[l]*crossprod(X_l))
            mu1 <- V%*%(Sigmainv_mu + tau[l]*crossprod(X_l, yt[z[i,] == l]))
            ...
            aux <- yt[z[i,] == l]- X_l%*%t(beta[l,,drop = FALSE])
            Tau[i,l] <- tau[l] <- rgamma(1, shape = a + (ns[l]/2), rate = b + 0.5*crossprod(aux))
        ...
        if(k == 1) {
            meanmu <- Vaux%*%(S_inv%*%m + Sigmainv[i-1,,]%*%sum(Beta[i,,]))
        } else {
            meanmu <- Vaux%*%(S_inv%*%m + Sigmainv[i-1,,]%*%apply(Beta[i,,],2,sum))
        }
        mu[i,] <- mvrnorm(1, mu = meanmu, Sigma = Vaux)
        
        Vaux1 <- 0
        for(l in 1:L) {
            Vaux1 <- Vaux1 + tcrossprod(Beta[i,l,]-mu[i,])
        }
